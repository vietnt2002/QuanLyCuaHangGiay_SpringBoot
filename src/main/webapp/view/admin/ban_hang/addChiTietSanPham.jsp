...
<h3 class="text-center mt-5 mb-5">Thêm mới chi tiết sản phẩm</h3>
<sf:form method="post" action="/store/chi-tiet-san-pham-store" modelAttribute="chiTietSanPham">
    ...
    <div class="input-group mb-3">
        ...
        <sf:select class="form-select" aria-label="Default select example" name="idKichThuoc" path="idKichThuoc">
            <c:forEach items="${listKichThuoc}" var="kichThuoc">
                <sf:option value="${kichThuoc.id}">${kichThuoc.ten}</sf:option>
            </c:forEach>
        </sf:select>
    </div>
