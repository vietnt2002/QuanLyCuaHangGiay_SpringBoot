...
<sf:form method="post" action="/hoa-don/update/${hoaDon.id}" modelAttribute="hoaDon">
        ...
        <sf:select class="form-select" aria-label="Default select example" name="idNV" path="idNV">
            <c:forEach items="${listNhanVien}" var="nhanVien">
                <option value="${nhanVien.id}"
                        <c:if test="${nhanVien.id == hoaDon.idNV.id}">selected</c:if>> ${nhanVien.ten}
                </option>
            </c:forEach>
        </sf:select>
    </div>
        ...
        <sf:select class="form-select" aria-label="Default select example" name="idKH" path="idKH">
            <c:forEach items="${listKhachHang}" var="khachHang">
                <option value="${khachHang.id}"
                        <c:if test="${khachHang.id == hoaDon.idKH.id}">selected</c:if>> ${khachHang.ten}
                </option>
            </c:forEach>
        </sf:select>
    </div>
